<script setup lang="ts">
import { ref } from "vue";

const tab = ref(null);
</script>

<template>
  <div>
    <div class="d-flex align-center pa-5">
      <v-avatar size="32" class="mr-2">
        <img src="@/assets/images/avatar-1.png" width="32" alt="Julia" />
      </v-avatar>
      <div>
        <h6 class="text-h6 mb-0">JWT User</h6>
        <p class="text-caption mb-0">UI/UX Designer</p>
      </div>
      <div class="ml-auto">
        <v-btn variant="text" color="primary" rounded="sm" icon size="small">
          <v-icon color="blue-darken-2" icon="mdi-logout" size="small"></v-icon>
        </v-btn>
      </div>
    </div>
    <v-tabs v-model="tab" color="primary" grow>
      <v-tab value="111">
        <v-icon
          color="blue-darken-2"
          icon="mdi-account-circle-outline"
          size="small"
        ></v-icon>

        Profile
      </v-tab>
      <v-tab value="222">
        <v-icon
          color="blue-darken-2"
          icon="mdi-cog-outline"
          size="small"
          class="v-icon--start"
        ></v-icon>
        Setting
      </v-tab>
    </v-tabs>
    <div style="height: calc(100vh - 300px); max-height: 240px">
      <v-window v-model="tab">
        <v-window-item value="111">
          <v-list class="py-0" aria-label="profile list" aria-busy="true">
            <v-list-item color="primary" rounded="0" value="Edit profile">
              <template v-slot:prepend>
                <span class="mr-4">
                  <v-icon
                    color="blue-darken-2"
                    icon="mdi-account-edit-outline"
                    size="small"
                  ></v-icon>
                </span>
              </template>

              <v-list-item-title class="text-h6">
                Edit Profile</v-list-item-title
              >
            </v-list-item>

            <v-list-item color="primary" rounded="0" value="View Profile">
              <template v-slot:prepend>
                <span class="mr-4">
                  <v-icon
                    color="blue-darken-2"
                    icon="mdi-account-circle-outline"
                    size="small"
                  ></v-icon>
                </span>
              </template>

              <v-list-item-title class="text-h6">
                View Profile</v-list-item-title
              >
            </v-list-item>

            <v-list-item color="primary" rounded="0" value="Social Profile">
              <template v-slot:prepend>
                <span class="mr-4">
                  <v-icon
                    color="blue-darken-2"
                    icon="mdi-account-group-outline"
                    size="small"
                  ></v-icon>
                </span>
              </template>

              <v-list-item-title class="text-h6">
                Social Profile</v-list-item-title
              >
            </v-list-item>

            <v-list-item color="primary" rounded="0" value="Billing">
              <template v-slot:prepend>
                <span class="mr-4">
                  <v-icon
                    color="blue-darken-2"
                    icon="mdi-wallet-outline"
                    size="small"
                  ></v-icon>
                </span>
              </template>

              <v-list-item-title class="text-h6"> Billing</v-list-item-title>
            </v-list-item>

            <v-list-item color="secondary" rounded="0">
              <template v-slot:prepend>
                <span class="mr-4">
                  <v-icon
                    color="blue-darken-2"
                    icon="mdi-logout"
                    size="small"
                  ></v-icon>
                </span>
              </template>

              <v-list-item-title class="text-h6"> Logout</v-list-item-title>
            </v-list-item>
          </v-list>
        </v-window-item>
        <v-window-item value="222">
          <v-list class="py-0" aria-label="profile list" aria-busy="true">
            <v-list-item color="primary" rounded="0" value="Support">
              <template v-slot:prepend>
                <span class="mr-4">
                  <v-icon
                    color="blue-darken-2"
                    icon="mdi-account-cog-outline"
                    size="small"
                  ></v-icon>
                </span>
              </template>

              <v-list-item-title class="text-h6"> Support</v-list-item-title>
            </v-list-item>

            <v-list-item color="primary" rounded="0" value="Account">
              <template v-slot:prepend>
                <span class="mr-4">
                  <v-icon
                    color="blue-darken-2"
                    icon="mdi-face-agent"
                    size="small"
                  ></v-icon>
                </span>
              </template>

              <v-list-item-title class="text-h6">
                Account settings</v-list-item-title
              >
            </v-list-item>

            <v-list-item color="primary" rounded="0" value="Privacy">
              <template v-slot:prepend>
                <span class="mr-4">
                  <v-icon
                    color="blue-darken-2"
                    icon="mdi-lock-outline"
                    size="small"
                  ></v-icon>
                </span>
              </template>

              <v-list-item-title class="text-h6">
                Privacy center</v-list-item-title
              >
            </v-list-item>

            <v-list-item color="primary" rounded="0" value="Feedback">
              <template v-slot:prepend>
                <span class="mr-4">
                  <v-icon
                    color="blue-darken-2"
                    icon="mdi-comment-outline"
                    size="small"
                  ></v-icon>
                </span>
              </template>

              <v-list-item-title class="text-h6"> Feedback</v-list-item-title>
            </v-list-item>

            <v-list-item color="primary" rounded="0" value="History">
              <template v-slot:prepend>
                <span class="mr-4">
                  <v-icon
                    color="blue-darken-2"
                    icon="mdi-format-list-bulleted"
                    size="small"
                  ></v-icon>
                </span>
              </template>

              <v-list-item-title class="text-h6"> History</v-list-item-title>
            </v-list-item>
          </v-list>
        </v-window-item>
      </v-window>
    </div>
  </div>
</template>
